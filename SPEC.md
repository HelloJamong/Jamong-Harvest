...
**향후 과제**: GitHub Actions에 자동 검증 step 추가 고려 (hook 동작 테스트)
...
### 스킬 자동 업데이트 (`update` 명령)

현재 `install.sh`는 설치만 지원함. 이미 설치된 스킬을 저장소 최신 버전으로 갱신하는 기능이 필요.

**요구사항**
- `./install.sh update claude` — Claude Code 경로의 기존 스킬을 덮어쓰기 갱신
- `./install.sh update all` — 전체 경로 갱신
- 설치된 스킬과 소스가 동일하면 skip (불필요한 복사 방지)
- 갱신 전후 변경 파일 목록 출력

**수용 기준 (Acceptance Criteria)**
1. `./install.sh update claude` 실행 시 변경된 스킬만 `✓ updated` 표시
2. 변경 없는 스킬은 `  [skip] no change` 표시
3. `install codex`와 동일한 경로 처리 로직 재사용
4. `bash -n install.sh` 통과
