...
> Jamong 전용 Codex/OMX 및 범용 코딩 에이전트 운영 템플릿입니다. 이 파일을 대상 프로젝트 루트의 `AGENTS.md`로 복사한 뒤 `<PROJECT_NAME>`과 `<PROJECT_ROOT>`를 실제 값으로 바꿔 사용합니다.
...
- Run commands from `<PROJECT_ROOT>` unless a task explicitly targets another directory.
